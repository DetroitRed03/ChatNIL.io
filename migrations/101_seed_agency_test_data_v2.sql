-- ============================================================================
-- Migration 101: Seed Agency Dashboard Test Data (v2)
-- ============================================================================
-- This uses existing athlete data and creates campaigns without requiring
-- a specific agency auth user

-- Step 1: Get an existing agency from the database or skip if none exists
-- We'll use any existing agency for testing purposes
DO $$
DECLARE
  v_agency_id UUID;
...
BEGIN
  ...
  -- If no agency exists, print message and exit
  IF v_agency_id IS NULL THEN
    RAISE NOTICE 'No agencies found. Please create an agency user first via the application.';
    RAISE NOTICE 'After creating an agency user, re-run this migration.';
    RETURN;
  END IF;
  ...
  RAISE NOTICE 'Created % campaigns', 4;
  ...
  -- Only proceed if we have athletes and campaigns
  ...
  RAISE NOTICE 'Created activity log entries';
