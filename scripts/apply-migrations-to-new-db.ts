/**
 * Apply All Migrations to NEW Supabase Database
 *
 * Loads credentials directly from .env.local to avoid shell caching
 */
...
async function main() {
  console.log('\n' + '='.repeat(70));
  log('🚀 Applying Migrations to NEW Supabase', '🚀');
  console.log('='.repeat(70) + '\n');

  // Load fresh environment
  const env = loadEnvFile();
  log(`Target: ${env.NEXT_PUBLIC_SUPABASE_URL}`, '🎯');
...
GRANT EXECUTE ON FUNCTION public.get_unread_count() TO authenticated;
  ...
  const { error } = await supabase.rpc('exec_sql', { query: consolidatedSQL });
  ...
  log('\n✅ Created:', '✅');
  log('  - agencies table', '  ');
  log('  - athlete_fmv_data table', '  ');
  log('  - agency_athlete_matches table', '  ');
  log('  - agency_athlete_messages table', '  ');
  log('  - quiz_questions table', '  ');
  log('  - conversation_summaries view', '  ');
  log('  - mark_messages_read() function', '  ');
  log('  - get_unread_count() function', '  ');
  log('  - All RLS policies', '  ');

  log('\n📍 Next: Run seeding script', '📍');
  log('  npx tsx scripts/seed-new-database.ts', '💡');
