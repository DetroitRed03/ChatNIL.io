...
BEGIN
  ...
  SELECT
    ...
    COUNT(CASE WHEN uqp.is_correct THEN 1 END)::integer,
    COALESCE(SUM(uqp.points_earned), 0)::integer,
    CASE WHEN COUNT(DISTINCT question_id) > 0
      THEN ROUND((COUNT(CASE WHEN uqp.is_correct THEN 1 END)::numeric / COUNT(DISTINCT question_id)::numeric * 100), 2)
      ELSE 0 END,
    COALESCE(SUM(uqp.time_taken_seconds), 0)::integer,
